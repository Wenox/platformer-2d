        ...
        gui.widgets[to_underlying(Loader::Btn::loadConfirm)]->connect("Pressed", [&]() {
                std::visit(overload{
                        [&](BmpReader &) {
                            ...
                            const auto& data = std::get<BmpReader>(mapReader.value()).getData();
                            const auto& encodedObjs = std::get<Encoder<PixelColor>>(encoder.value()).encodedObjects;
                            ...
                            state::gameID = stateMachine.insert(std::make_shared<StateGame>(stateMachine, resources, blocksNum));
                            stateMachine = state::gameID;
